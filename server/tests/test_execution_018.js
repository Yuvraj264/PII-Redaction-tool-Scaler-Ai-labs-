...
(async () => {
  ...
  await runAsyncTest('TEST 4: OpenXML DOCX Structural Paragraph & Table Cell Parsing', async () => {
    ...
    assert.ok(parsedDoc.metadata.paragraphsCount > 500, 'Must extract over 500 paragraphs');
  });
  ...
  await runAsyncTest('TEST 5: Full 9 PII Detector Execution', async () => {
    ...
    assert.strictEqual(detectionResult.success, true);
    assert.ok(detectionResult.summary.totalEntitiesDetected > 1000);
    assert.ok(detectionResult.summary.breakdown['PERSON'] > 0);
    assert.ok(detectionResult.summary.breakdown['EMAIL'] > 0);
    assert.ok(detectionResult.summary.breakdown['PHONE'] > 0);
    assert.ok(detectionResult.summary.breakdown['ORGANIZATION'] > 0);
  });
  ...
  await runAsyncTest('TEST 10: Path Traversal Attack Rejection Audit (../../file)', async () => {
    ...
    const res = await new Promise((resolve, reject) => {
      http.get(`http://127.0.0.1:${port}/api/documents/${encodeURIComponent(maliciousDocId)}/download`, (res) => {
        let body = '';
        res.on('data', chunk => body += chunk);
